...
fixed there, because fixing each one changes behaviour. **All eleven are
fixed now**, each in its own commit, on top of the refactor. The files are kept as
...
| 11 | [Two dialogs have no title, so a screen reader announces nothing](11-dialogs-without-a-title.md) | Low-medium: the two screens a new user meets first | `9cd9d1c` |
