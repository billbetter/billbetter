...
const TONES = {
  none: {
    bar: "bg-sand-300 dark:bg-surface/20",
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div
            key={i}
            className="relative h-1.5 overflow-hidden rounded-[2px] bg-sand-200 dark:bg-surface/12 dark:dark:bg-surface-inverted/12"
          >
            <motion.span
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-[11.5px] leading-5 text-warning-600 dark:text-warning-400"
          initial={false}
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-[4px] border border-sand-200 text-content-inverted dark:border-content-inverted/[0.16] dark:text-sand-900">
                <motion.span
                  className="absolute inset-0 rounded-[3px] bg-success-500"
                  initial={false}
